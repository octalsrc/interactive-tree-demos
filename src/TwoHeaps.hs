...
main = do let n = "main"
          ...
          sc1 <- startCanvas n
                             ...
                             (const (return ()))
          sc2 <- startCanvas "second"
                             (900,500)
                             s
                             ["main","message","frame","defbuttons"]
                             (const (return ()))
          startHeapGames (sc1,sc2,conf)
...
startHeapGames (sc1,sc2,conf) = 
  do g <- newStdGen
     (gameManips1,runManip1) <- newAddHandler
     (gameManips2,runManip2) <- newAddHandler
     let runManip a = runManip1 a >> runManip2 a
         env1 = Env sc1 conf runManip1
         env2 = Env sc2 conf runManip2
     attachButton (newGameButtonID conf) 
                  (doNewGame env1 >> doNewGame env2) (const return ())
     startGame env1 gameManips1 g
     startGame env2 gameManips2 g

doNewGame env = do g <- newStdGen
                   n <- restartGame env <$> readNewGame env g
                   (runM env) n
...
readNewGame :: Env -> StdGen -> IO GameState
readNewGame env g = 
